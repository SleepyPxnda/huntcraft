...
class PlayerManager {
    ...
    fun loadNewPlayer(player: Player){
        ...
        val existingDatabasePlayer = PlayerTable.selectAll().where { PlayerTable.uuid eq player.uniqueId }.firstOrNull()

        val ongoingQuestsForPlayer = QuestProgressTable.selectAll()
        ...
        val completedQuestsForPlayer = CompletedQuestTable.innerJoin(QuestTable)
            .selectAll()
            .where { (QuestProgressTable.playerUuid eq player.uniqueId) }
            .map {
                QuestDTO(
